...
const props = withDefaults(
  ...
  {
    fixedColumns: [] // Значение по умолчанию для fixedColumns
  }
);
...
const isFixedColumn = (key: string) => {
  return props.fixedColumns?.includes(key) ?? false
}

const fixedColumnLeft = (index: number) => {
  if (!props.fixedColumns || props.fixedColumns.length === 0) return 'auto';

  const widths = props.columns.slice(0, index).map((col) => parseInt(col.width || '80', 10));

  return widths.reduce((sum, width) => sum + width, 0) + 'px';
};
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full bg-white border border-gray-200">
      ...
      <tr>
        <th
          v-if="selectable"
          class="px-5 py-4 text-left font-medium border-b border-gray-200 bg-gray-100 relative max-w-[30px] sticky left-0 z-10"
        >
        ...
        <th
          ...
          :style="{
            ...
            left: index === 0 && isFixedColumn(column.key) ? '-1px' : (index === 1 && isFixedColumn(column.key) ? `calc(${fixedColumnLeft(index)} + 2px)` : fixedColumnLeft(index)),
            boxShadow: isFixedColumn(column.key) && index === props.fixedColumns.length - 1 ? '2px 0 5px rgba(0, 0, 0, 0.1)' : 'none'
          }"
          :class="[
            'px-6 py-3 border-b border-gray-200 bg-gray-100 text-left text-sm text-primary cursor-pointer',
            isFixedColumn(column.key) ? 'sticky z-10' : ''
          ]"
      ...
      <tr v-for="(row, rowIndex) in sortedRows" :key="rowIndex">
        <td
          v-if="selectable"
          class="px-5 py-4 border-b max-w-[30px] whitespace-nowrap sticky left-0 z-10 bg-white border-l border-gray-200"
        >
          <input
        ...
        <td
          ...
          :style="{
            width: column.width || 'auto',
            left: colIndex === 0 && isFixedColumn(column.key) ? '-1px' : (colIndex === 1 && isFixedColumn(column.key) ? `calc(${fixedColumnLeft(colIndex)} + 2px)` : fixedColumnLeft(colIndex)),
            backgroundClip: 'padding-box',
            boxShadow: isFixedColumn(column.key) && colIndex === props.fixedColumns.length - 1 ? '2px 0 5px rgba(0, 0, 0, 0.1)' : 'none'
          }"
          :class="[
            'px-6 py-4 text-sm font-medium text-gray-900 border-b border-gray-200',
            column.class,
            isFixedColumn(column.key) ? 'sticky z-20 bg-white' : '',
            fixedColumnLeft(colIndex) === '0px' ? 'border-l border-gray-200' : ''
          ]"
        >
